# CTI Agent v2.0 Design Document
...
Three sources generate detection rules (CTI Agent's `generate-report`, gl0bal01's `detection-engineer`, YARAHQ's `yara-rule-skill`). Resolution:

1. **YARA rules**: YARAHQ skill is the primary author. gl0bal01's detection-engineer defers to it. CTI Agent's generate-report calls the YARAHQ skill rather than generating rules inline.
2. **Sigma rules**: gl0bal01's detection-engineer is the primary author. CTI Agent's generate-report calls it rather than generating rules inline.
3. **Report generation**: CTI Agent's `generate-report` remains the primary report skill. gl0bal01's `malware-report-writer` is used only for malware-specific deep-dive reports.

### TI Mindmap HUB Deconfliction

The TI Mindmap HUB MCP server performs Diamond Model analysis and STIX 2.1 generation -- capabilities that overlap with CTI Agent's core skills. Boundaries:

- **Use TI Mindmap HUB as**: A source of pre-analyzed intelligence, a STIX 2.1 validator, and a CVE intelligence feed
- **Do NOT use it as**: A replacement for CTI Agent's own Diamond Model or ACH analysis
- **Rationale**: CTI Agent's analysis skills are the core differentiator. Delegating analysis to an external MCP server reduces the project to a wrapper.
...
```json
{
  "name": "Campaign X - CTI Agent Analysis",
  "versions": {"layer": "4.5", "attack": "15"},
  "domain": "enterprise-attack",
  "description": "Auto-generated by CTI Agent from Diamond Model analysis",
  "techniques": [
    {
...
git init
git add .
git commit -m "CTI Agent v1.0.0 - Initial commit"
```
...
Addressed in Section 2 (Skill Conflict Resolution). Clear ownership: YARAHQ owns YARA, gl0bal01's detection-engineer owns Sigma, CTI Agent's generate-report orchestrates but delegates.
...
**Demo mode activation**: Set `CTI_AGENT_DEMO_MODE=true` in environment. Skills use mock MCP responses instead of live API calls.
...
*CTI Agent v2.0 Design Document -- 2026-02-16*
