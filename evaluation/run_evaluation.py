...
def run_python_grader(script: str, source_path: str, output_path: str, requires_source: bool) -> Dict[str, Any]:
...
def heuristic_quality_score(output: str, config: Dict) -> float:
    ...
    """
    positive_count = sum(1 for indicator in config["quality_indicators"]["positive"] 
                        if indicator.lower() in output.lower())
    negative_count = sum(1 for indicator in config["quality_indicators"]["negative"]
                        if indicator.lower() in output.lower())
    
    # Base score from positive indicators
    base_score = min(positive_count / len(config["quality_indicators"]["positive"]), 1.0)
...
def run_all_graders(source_path: str, output_path: str) -> Dict[str, Any]:
    ...
    results["aggregate"] = {
        "weighted_average": round(weighted_sum / total_weight, 2) if total_weight > 0 else 0.0,
        "simple_average": round(
            sum(r.get("score", 0) for r in results["grader_results"].values()) / len(GRADERS), 
            2
        ),
    ...
    if json_output:
        ...
        if agg["failures"]:
            ...
            for f in agg["failures"]:
                print(f"  • {f['grader']}: {f['score']:.2f} < {f['threshold']} (gap: {f['gap']})")
