...
REQUIRED_SECTIONS: List[Tuple[str, str]] = [
    ...
    (
        r"(?i)(indicators?\s+of\s+compromise|iocs?|network\s+indicators?|file\s+indicators?)",
        "IOCs",
    ),
    (
        r"(?i)(defensive\s+(recommendations?|actions?)|mitigations?|countermeasures?)",
        "Recommendations",
    ),
    (r"(?i)((key\s+)?assumptions?|limitations?|caveats?)", "Assumptions"),
    (
        r"(?i)((intelligence\s+)?gaps?|unknown|missing\s+information)",
        "Intelligence Gaps",
    ),
...
def get_section_details(output: str) -> dict:
    ...
    details = {"present": [], "missing": [], "total_required": len(REQUIRED_SECTIONS)}
